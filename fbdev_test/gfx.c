 ...
 * 08-29-25 E. Brombaugh - updated with fixes from ch32/nl_bg work
 */
...
uint16_t gfx_chrbuff[2][64];	// double buffered in case of DMA
 ...
 * initialize display
 */
uint8_t gfx_init(GFX_DRIVER *drvr)
{
	gfxdrv = drvr;

	uint8_t result = gfxdrv->init();
	if(result)
	{
		return result;
	}
	
	gfxdrv->setRotation(1);
...
{
	gfxdrv->fillRect(x0, y, x1-x0+1, 1, forecolor);
}
...
{
	gfxdrv->fillRect(x, y0, 1, y1-y0+1, forecolor);
}
 ...
 */
void gfx_drawroundedrect(GFX_RECT *rect, int16_t radius)
{
	/* check for inversion */
	if(rect->x0 > rect->x1)
		gfx_swap(&rect->x0, &rect->x1);
	...
	int16_t min = rect->x1 - rect->x0;
	radius = (2*radius < min) ? radius : min/2;
	min = rect->y1 - rect->y0;
	radius = (2*radius < min) ? radius : min/2;

	/* draw H and V segments */
	gfxdrv->fillRect(rect->x0+radius, rect->y0, rect->x1-rect->x0-2*radius+1, 1, forecolor);
	gfxdrv->fillRect(rect->x0+radius, rect->y1, rect->x1-rect->x0-2*radius+1, 1, forecolor);
	gfxdrv->fillRect(rect->x0, rect->y0+radius, 1, rect->y1-rect->y0-2*radius+1, forecolor);
	gfxdrv->fillRect(rect->x1, rect->y0+radius, 1, rect->y1-rect->y0-2*radius+1, forecolor);
	
	/* draw corners by Bresenham algorithm */
    int16_t x_pos = -radius;
    int16_t y_pos = 0;
    int16_t err = 2 - 2 * radius;
    int16_t e2;
    ...
    {
		/* UL, LL, UR, LR */
        gfxdrv->drawPixel(rect->x0+radius + x_pos, rect->y0+radius - y_pos, forecolor);	// UL
        gfxdrv->drawPixel(rect->x0+radius + x_pos, rect->y1-radius + y_pos, forecolor);	// LL
        gfxdrv->drawPixel(rect->x1-radius - x_pos, rect->y0+radius - y_pos, forecolor);	// UR
        gfxdrv->drawPixel(rect->x1-radius - x_pos, rect->y1-radius + y_pos, forecolor);	// LR
        e2 = err;
        if(e2 <= y_pos)
        {
            err += ++y_pos * 2 + 1;
 ...
 * xor mode 2 is not a good fit for this...
 */
void gfx_fillroundedrect(GFX_RECT *rect, int16_t radius)
{
	/* check for inversion */
	if(rect->x0 > rect->x1)
		gfx_swap(&rect->x0, &rect->x1);
	...
	/* central box */
	gfxdrv->fillRect(rect->x0, rect->y0+radius, rect->x1-rect->x0+1,
		(rect->y1-radius)-(rect->y0+radius)+1, forecolor);
	
	/* draw corners + upper/lower boxes by Bresenham algorithm */
    int16_t x_pos = -radius;
    int16_t y_pos = 0;
    int16_t err = 2 - 2 * radius;
    int16_t e2;
    ...
    do
    {
		gfxdrv->fillRect(rect->x0+radius + x_pos, rect->y0+radius - y_pos,
			(rect->x1-radius-x_pos)-(rect->x0+radius+x_pos)+1, 1, forecolor); // UL - UR
		gfxdrv->fillRect(rect->x0+radius + x_pos, rect->y1-radius + y_pos,
			(rect->x1-radius-x_pos)-(rect->x0+radius+x_pos)+1, 1, forecolor); // LL - LR
        e2 = err;
        if(e2 <= y_pos)
        {
            err += ++y_pos * 2 + 1;
...
void gfx_drawchar_1x(int16_t x, int16_t y, uint8_t chr)
{
	...
	for(i=0;i<8;i++)
	{
		...
		for(j=0;j<8;j++)
		{
            ...
            /* update x counter and clip x to right */
            xt++;
            if(xt>(gfxdrv->xmax-1))
                break;
		}

        /* update y counter and clip y to bottom */
        yt++;
        if(yt>(gfxdrv->ymax-1))
            break;
	}
